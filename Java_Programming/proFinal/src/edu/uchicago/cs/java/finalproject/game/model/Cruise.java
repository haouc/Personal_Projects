...
public class Cruise extends Sprite {
	...
	private final double FIRE_POWER = 50;
	...
	public void move() {
        ...
        for(Movable t: CommandCenter.movFoes){
            if (temp > t.getCenter().distance(this.getCenter()))
            temp = t.getCenter().distance(this.getCenter());
        }
		
		if (getExpire() < MAX_EXPIRE -5){
            double dAdjustX = Math.cos(Math.toRadians(Math.random()*360));

            double dAdjustY = Math.sin(Math.toRadians(Math.random()*360));

			setDeltaX(getDeltaX() * 1.07 + dAdjustX);
			setDeltaY(getDeltaY() * 1.07 + dAdjustY);
		}
