 ...
 public class GamePanel extends Panel {
	...
	private void drawScore(Graphics g) {
		...
		if (CommandCenter.getScore() != 0) {
			g.drawString("SCORE :  " + CommandCenter.getScore() + "   ★☺☺★   " + "Falcon: "
                    + CommandCenter.getNumFalcons() + "   ★☺☺★   "  + "Level: " + CommandCenter.getLevel()
...
//		grpOff.fillRect(0, 0, Game.DIM.width, Game.DIM.height);
        loadImage();
        grpOff.drawImage(backGround,0,0,this);
     ...
     public void loadImage()
     {
         if(backGround == null)
         {
             ...
             try
             {
                 backGround = ImageIO.read(new File("src/edu/uchicago/cs/java/finalproject/images/u0.jpg"));
             } catch (IOException e)
             {
           ...
           {
             int pic = CommandCenter.getLevel()%21;
             try
             {
                 backGround = ImageIO.read(new File("src/edu/uchicago/cs/java/finalproject/images/u" + pic + ".jpg"));
             } catch (IOException e)
             {
                 System.out.println("No image");
                 JOptionPane.showMessageDialog(this, "Can't find image!", "file not found", JOptionPane.ERROR_MESSAGE);
	...
	private void displayTextOnScreen() {
        strDisplay = "JAVA PROGRAMMING FINAL PROJECT";
        grpOff.drawString(strDisplay,
                (Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
                        + nFontHeight - 40);
        strDisplay = "STAR WAR";
		...
		strDisplay = "left index finger on 'F' for Missile";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 320);
